...
int main()
{
    char func[10];
    void *bib;
    bib = dlopen("/home/gleb/eltex/2/6.2/liblistFunc.so", RTLD_NOW);
    ...
    FindInList = dlsym(bib, "FindInList");
    EditListValue = dlsym(bib, "EditListValue");
    InsertSort = dlsym(bib, "InsertSort");
    DeleteValue = dlsym(bib, "DeleteValue");
    DeleteList = dlsym(bib, "DeleteList");
    PrintList = dlsym(bib, "PrintList");
    PrintValue = dlsym(bib, "PrintValue");
    delete_n = dlsym(bib, "delete_n");
    isempty = dlsym(bib, "isempty");
    Contact *head = NULL;
    Contact *tail = NULL;
    while(1)
    {
